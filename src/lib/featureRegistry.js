    ...
    guideContent: {
      ...
      play: [
        ...
        },
        {
          ...
          body: '상단 [🧩 이모티콘 시트 분할기]는 가로가 긴 2열 스튜디오입니다. 시트를 올리면 줌 35% → 모드 A 스마트 감지 → 모드 B 절단선 세팅이 한 번 실행됩니다. 렌더는 Crop(절단 경계만) → 360 모서리 Flood-Fill(T=18) → 하단 어두운 획 1:1 치환 순입니다. 텍스트 구간에 fillRect 흰 패치를 쓰지 않습니다. Outline은 기본 ON입니다. [🐞 진단 로그]는 칸마다 모서리 알파·흰 사각·인접 행 침범·하이라이트 보존을 JSON으로 복사하고 F12 콘솔 테이블을 띄웁니다.',
