...
export const APP_FEATURES_REGISTRY = [
  ...
  feature({
    ...
    description: 'Crop→Flood T=18→획치환, 가로·세로 결합 분할, 구멍 투명화 OFF 기본, 상/하단 텍스트, 체커보드 배경, 인스펙터, 줌 35%, Outline ON',
    ...
    guideContent: {
      ...
      play: [
        ...
        },
        {
          ...
          tip: '투명 누끼가 검게 보이면 탐색기 미리보기 특성입니다. 분할기 [배경 모드]를 체커보드/다크/라이트로 바꿔 실제 알파를 확인하세요.',
        ...
        },
        {
          ...
          body: '문제가 있으면 [🐞 진단 로그] 한 번으로 칸마다 모서리 알파(PASS/FAIL)·흰 사각·인접 행 침범·하이라이트 보존 JSON이 클립보드에 복사되고 F12 콘솔 테이블이 뜹니다. 줌 옆 [배경 모드]는 🏁 체커보드 → ⬛ 다크 → ⬜ 라이트로 순환해 360 썸네일과 메인 뷰포트 대비를 즉시 봅니다. 결함이 있으면 하단 18단계 Live HUD에 슬라이스 경고가 뜹니다.',
