...
export const APP_FEATURES_REGISTRY = [
  ...
  feature({
    ...
    description: '1px 초슬림 절단선, 외곽 재단, 슈퍼샘플링 샤프닝, 텍스트 가독성 보정, JSZip 카카오 규격 점검',
    ...
    guideContent: {
      ...
      play: [
        ...
        },
        {
          ...
          body: '상단 [🧩 이모티콘 시트 분할기]에서 시트를 올립니다. 모드 A는 객체를 자동으로 찾고, 모드 B는 1px(0.5mm 체감) 시안/마젠타 절단선을 드래그해 칸을 맞춥니다. 금색 외곽 재단선 4개로 상하좌우 여백을 자르고, [+ 가로선 추가]/[+ 세로선 추가]와 선 끝 ✖(또는 우클릭)으로 절단선을 늘리거나 지웁니다. 분할은 3배 슈퍼샘플링 후 단계적 다운스케일·어댑티브 언샵 마스크로 외곽선을 살립니다. 상단 [텍스트 가독성 보정]에서 원본/블랙 강화/화이트/커스텀 색과 1px Outline Assist를 고른 뒤 [📦 전체 ZIP 다운로드 (카카오 규격 360x360)]로 받습니다.',
          params: ['1px 고대비 절단선', '외곽 재단선 4축', '선 추가/삭제', '3× 슈퍼샘플링', '텍스트 톤 · Outline Assist'],
          tip: '외곽 금색 선으로 시트 여백을 먼저 자르고, 안쪽 절단선을 맞추세요. 슬라이더를 다시 움직이면 그 축은 재단 범위 안 균등 분할로 돌아갑니다.',
  ...
  feature({
    ...
    guideContent: {
      ...
      summary: [
        ...
        { title: '내보내기·단축키', body: '우측에서 투명 PNG, JPEG, 모션 GIF, ICO를 고릅니다. PNG는 1x/2x/4x 배율을 고른 뒤 받습니다. GIF는 네온 펄스·소프트 플로팅·시네마틱 페이드 중 하나를 고른 뒤 [🎬 GIF 다운로드]로 받습니다. AI 이모티콘 시트는 상단 [🧩 이모티콘 시트 분할기]에서 나누며, 모드 B는 1px 절단선·금색 외곽 재단·선 추가/삭제로 칸을 맞춘 뒤 슈퍼샘플링 360×360 ZIP으로 받습니다. 방향키 1px, Shift+방향키 10px, Ctrl+Z 실행 취소, Ctrl+Y 다시 실행, Delete는 추가 레이어만 지웁니다(메인/서브 잠금).' },
      ],
    },
  }),
  feature({
