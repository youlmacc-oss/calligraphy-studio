...
export const APP_FEATURES_REGISTRY = [
  ...
  feature({
    id: 'bg',
    name: '배경 합성 & 이미지 로더',
    description: 'FileReader 로컬 배경 이미지 로드 및 블렌드 모드(Multiply/Screen) 연산 검사',
  ...
  feature({
    id: 'live-hud',
    name: '실시간 텍스트 인포 바',
    description: '선택 레이어 글자 수·폰트·좌표 HUD 및 중앙 캔버스 뷰포트 정렬 검사',
    ...
    guideContent: {
      ...
      play: [
        {
          title: '캔버스 아래 큰 인포 바를 본다',
          body: '미리보기 바로 아래에 선택 레이어의 [👑 메인] / [✨ 서브] 배지, 글자 수/줄 수, 폰트명·크기·자간·행간, 좌표·회전·투명도, 적용 프리셋이 실시간으로 뜹니다. 폭은 중앙 열 전체를 쓰고, 글자는 시스템 고딕(산세리프) 16px 전후입니다.',
          params: ['글자 수 / 줄 수', '폰트 · px · 자간 · 행간', 'X/Y · 회전 · 투명도', '프리셋 이름'],
          tip: '레이어 카드를 바꾸거나 캔버스에서 글자를 클릭하면 HUD가 그 레이어로 바뀝니다. 폰트 목록에 호버하면 미리보기 폰트명도 따라갑니다.',
          fail: '수치가 안 바뀌면 다른 레이어가 선택된 것입니다. 왼쪽 카드의 포커스 배지를 확인하세요.',
        },
        {
          title: '해상도가 달라도 캔버스는 가운데',
          body: '중앙 작업 영역은 flex로 미리보기를 정중앙에 둡니다. #main-canvas-area가 좌표 기준점이라, 모니터 배율이나 창 크기가 달라도 글자가 좌상단으로 쏠리지 않습니다.',
          params: ['#main-canvas-area = 상대 좌표 원점', '드래그 = getBoundingClientRect 비율 매핑'],
          tip: '다른 PC에서 글자가 한쪽으로 붙으면 자가진단의 인포 바/뷰포트 항목을 먼저 돌려 보세요.',
          fail: '브라우저 확대만 과도하면 칸이 줄바꿈됩니다. 인포 바는 좁은 화면에서 2열·1열로 접히며 글자 크기는 유지됩니다.',
        },
      ],
    },
  }),
  ...
  feature({
    ...
    guideContent: {
      ...
      summary: [
        ...
        { title: '내보내기·단축키', body: '우측에서 투명 PNG, JPEG, 모션 GIF, ICO를 고릅니다. GIF는 네온 펄스·소프트 플로팅·시네마틱 페이드 중 하나를 고른 뒤 [🎬 GIF 다운로드]로 받습니다. AI 이모티콘 시트는 상단 [🧩 이모티콘 시트 분할기]에서 360×360 ZIP으로 나눕니다. Ctrl+Z 실행 취소, Ctrl+Y 다시 실행, Delete는 추가 레이어만 지웁니다(메인/서브 잠금).' },
      ],
    },
  }),
  feature({
