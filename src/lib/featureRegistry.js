...
export const APP_FEATURES_REGISTRY = [
  ...
  feature({
    ...
    description: '와이드 모달 줌/팬, 1px 절단선, 하단 35% 국소 텍스트 재채색, 슈퍼샘플링, JSZip 카카오 규격 점검',
    ...
    guideContent: {
      ...
      play: [
        ...
        },
        {
          ...
          body: '상단 [🧩 이모티콘 시트 분할기]는 96vw 와이드 창에서 시트를 올립니다. 미리보기 상단 [−] [100%] [+] [화면맞춤]으로 확대하고, 확대 후 빈 곳을 드래그해 이동합니다. 모드 A는 객체를 자동으로 찾고, 모드 B는 1px 절단선·금색 외곽 재단·선 추가/삭제로 칸을 맞춥니다. 상단 [텍스트 가독성 보정]은 360×360 하단 35%(Y 65~100%)의 글자 클러스터만 바꾸고 캐릭터 본체는 보존합니다. 분할은 3배 슈퍼샘플링 후 ZIP으로 받습니다.',
          params: ['와이드 모달 · 줌/팬', '1px 절단선 · 외곽 재단', '하단 35% 텍스트만 재채색', '3× 슈퍼샘플링', 'Outline Assist'],
          tip: '색을 바꿀 때는 캐릭터 얼굴이 아니라 말풍선·하단 글자만 바뀌어야 합니다. 절단선은 확대한 뒤 드래그하세요.',
  ...
  feature({
    ...
    guideContent: {
      ...
      summary: [
        ...
        { title: '내보내기·단축키', body: '우측에서 투명 PNG, JPEG, 모션 GIF, ICO를 고릅니다. PNG는 1x/2x/4x 배율을 고른 뒤 받습니다. GIF는 네온 펄스·소프트 플로팅·시네마틱 페이드 중 하나를 고른 뒤 [🎬 GIF 다운로드]로 받습니다. AI 이모티콘 시트는 상단 [🧩 이모티콘 시트 분할기] 와이드 창에서 줌/팬으로 칸을 맞춘 뒤, 하단 텍스트만 색 보정한 360×360 ZIP으로 받습니다. 방향키 1px, Shift+방향키 10px, Ctrl+Z 실행 취소, Ctrl+Y 다시 실행, Delete는 추가 레이어만 지웁니다(메인/서브 잠금).' },
      ],
    },
  }),
  feature({
