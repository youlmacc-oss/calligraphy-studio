...
export const APP_FEATURES_REGISTRY = [
  ...
  feature({
    ...
    description: '모드 B 엄격 셀 경계, 업로드 자동 트리거, 기본 줌 35%, Outline 기본 ON, 플러드필 T=22, 텍스트 감지 높이 슬라이더',
    ...
    guideContent: {
      ...
      play: [
        ...
        },
        {
          ...
          body: '상단 [🧩 이모티콘 시트 분할기]는 가로가 긴 2열 스튜디오입니다. 시트를 올리면 줌 35% → 모드 A 스마트 감지 → 모드 B 절단선 세팅이 한 번 실행됩니다. Outline 외곽선 보강은 기본 ON입니다. 모드 B는 절단선과 외곽 재단선 사이만 자르며 아래 행으로 높이를 늘리지 않습니다. 텍스트 색 치환은 [↕ 텍스트 감지 높이] 슬라이더(기본 하단 20%, 5~50%) 안의 어두운 글자 획만 바꿉니다. 흰 사각 패치를 칠하지 않습니다. 360 렌더 직후 네 모서리 플러드필(허용 오차 22)이 외곽 배경과 글자 주변 흰 판을 지우고, 진한 외곽선 안 하이라이트는 남깁니다. ZIP·썸네일은 그 최종 캔버스 PNG입니다.',
          params: ['모드 B 엄격 셀 경계 Y1~Y2', '업로드 후 자동 트리거 1회', '기본 줌 35%', 'Outline 기본 ON', '모드 A CCA 원형 보존', '플러드필 T=22 모서리 BFS', '글자 획만 색 치환(흰 패치 없음)', '텍스트 감지 높이 5~50%(기본 20%)', 'toBlob RGBA PNG'],
