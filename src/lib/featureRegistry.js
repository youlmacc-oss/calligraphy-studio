...
export const APP_FEATURES_REGISTRY = [
  ...
  feature({
    ...
    description: 'Crop→Flood T=18→획치환 파이프라인, 진단 인스펙터, 모드 B 엄격 셀, 기본 줌 35%, Outline 기본 ON',
    ...
    guideContent: {
      ...
      play: [
        ...
        },
        {
          ...
          body: '상단 [🧩 이모티콘 시트 분할기]는 가로가 긴 2열 스튜디오입니다. 시트를 올리면 줌 35% → 모드 A 스마트 감지 → 모드 B 절단선 세팅이 한 번 실행됩니다. 모드 A는 감지 알고리즘을 바꾸지 않고, 평균 높이의 1.7배를 넘는 세로 결합 덩어리만 반으로 나눕니다. 렌더는 Crop(절단 경계만) → 360 모서리 Flood-Fill(T=18) → 하단 어두운 획 1:1 치환 순입니다. 텍스트 구간에는 fillRect/clearRect 흰 사각을 쓰지 않고, 미색·흰 판 픽셀만 투명으로 펀치한 뒤 어두운 글자 픽셀 RGB만 직접 바꿉니다. Outline은 기본 ON입니다. [🐞 진단 로그]는 칸마다 모서리 알파(PASS/FAIL)·흰 사각·인접 행 침범·하이라이트 보존을 JSON으로 복사하고 F12 콘솔 테이블을 띄웁니다. 결함이 있으면 하단 18단계 Live HUD에 슬라이스 경고가 뜹니다.',
          params: ['모드 A 세로 결합 1.7배 후처리 분할', '흰 사각 패치 픽셀 펀치', 'Crop → Flood T=18 → 획 1:1 치환', '🐞 진단 로그 JSON', '모드 B 엄격 셀 경계 Y1~Y2', '기본 줌 35%', 'Outline 기본 ON', '모드 A CCA 원형 보존', '텍스트 감지 높이 5~50%(기본 20%)', 'toBlob RGBA PNG'],
  ...
  feature({
    ...
    guideContent: {
      ...
      summary: [
        ...
        { title: '내보내기·단축키', body: '우측에서 투명 PNG, JPEG, 모션 GIF, ICO를 고릅니다. PNG는 1x/2x/4x 배율을 고른 뒤 받습니다. GIF는 네온 펄스·소프트 플로팅·시네마틱 페이드 중 하나를 고른 뒤 [🎬 GIF 다운로드]로 받습니다. AI 이모티콘 시트는 상단 [🧩 이모티콘 시트 분할기]에서 모드 A 스마트 감지 또는 모드 B 그리드로 나눈 뒤, 투명 배경 360×360 PNG ZIP으로 받습니다. 방향키 1px, Shift+방향키 10px, Ctrl+Z 실행 취소, Ctrl+Y 다시 실행, Delete는 추가 레이어만 지웁니다(메인/서브 잠금).' },
      ],
    },
  }),
  feature({
