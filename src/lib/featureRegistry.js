...
export const APP_FEATURES_REGISTRY = [
  ...
  feature({
    id: 'guide-basics',
    name: '간단 설정 요약',
    description: '폰트, 컬러, 슬라이더, 레이아웃, 내보내기 압축 안내 · 기본 단색은 메인 셰이더 끊기 · 수묵 잉크는 텍스트 색 틴트 · 한자 노토 세리프 폴백',
    guideContent: {
      chapterId: 'basics',
      order: 10,
      ...
      summary: [
        ...
        { title: '컬러 픽커', body: '카드 하단 [텍스트 / 외곽선 / 그림자] 색 칸을 누릅니다. [기본 단색]은 메인 레이어도 19종/수묵 셰이더를 끊고 텍스트 색만 씁니다. [수묵 캘리그라피]는 검정 고정이 아니라 텍스트 색을 잉크 틴트로 씁니다. 한자(夢 등)는 선택 폰트에 글자가 없으면 노토 세리프 KR/SC로 같은 색을 그립니다. 외곽선 두께 0이면 테두리가 없고, 2~6이 포스터에 무난합니다. 그림자 블러는 8~18이 읽기 좋습니다. 두께·블러를 밀면 본문과 테두리가 같은 자리에 겹치고, 외곽선 모양의 유령 그림자는 생기지 않습니다.' },
  ...
  feature({
    ...
    description: '와이드 95vw×92vh · 자막 폰트 10선 · 개발 뷰 1600 스케일 · 모션 없음 기본 · 내 PC 업로드 클릭/드롭 · 인코딩 진행률 · 단일 이미지 RAF 루프 · 파티클 4종 · 핑퐁 루프 · 채팅 시뮬 · 심사 스펙 HUD · 클립 저장 · 초고화질 무한루프=GIF 내보내기 · 360 GIF/WebP · ZIP motion-01 · checkerboard-bg · bg-white 금지',
    ...
    guideContent: {
      ...
      play: [
        ...
        },
        {
          ...
          body: '본체 그래픽이나 이미지 1장만 있어도 중앙 프리뷰가 가상 1프레임으로 바로 루프됩니다. SOURCES에서 [본체 그래픽]을 고르면 분할 컷 뱅크와 시트 분할기는 숨기고 단일 본체 모션·픽셀 추출만 보이며, [이모티콘 컷]을 고른 뒤에만 분할 컷 목록이 편성됩니다. 우측 [🪄 캐릭터 영역 자동 추출]을 누르면 1.5초간 네온 점선 박스로 분리 영역을 보여 주고, 이후 순수 캐릭터에만 모션이 걸립니다. 스타일러에서 모션 스튜디오로 넘기는 본체는 가이드 십자·체커보드를 빼 순수 투명 PNG로 보내고, 수신단에서 외곽 가짜 격자/가이드 픽셀을 다시 지웁니다. [✨ 초정밀 픽셀 추출]은 가로 최대 팝업에서 좌·우 툴바를 양 끝에 붙이고 그 사이 중앙 캔버스가 원본 비율을 유지한 채 남은 폭을 채우며(와이드/세로/정사각), [🔍 정밀확대]로 클릭 지점 50×50 돋보기 격자를 연 뒤 적용합니다. [🔄 원본 복원]으로 추출을 즉시 되돌립니다. 자막 ON 후 입력창 옆 폰트 셀렉터에서 주아·도현·쿠키런·몬소리·G마켓·빙그레·연성·봄바람·배찌·프리텐다드 10선을 고르면 미리보기와 GIF/WebP가 같은 웹폰트로 그려집니다. [💬 꼬리 ON]을 켜면 말풍선 본체와 꼬리가 한 패스로 이어지고, 청록 끝점·노란 밑변 두 점을 드래그해 방향과 곡률을 맞춥니다. 내보내기에는 핸들이 빠집니다. 자막 ON 후 메인 캔버스에서 말풍선을 드래그해 옮길 수 있습니다. [내 PC 업로드] 탭을 누르거나 점선 상자를 클릭하면 파일 탐색기가 열리고, PNG/JPG/WebP를 여러 장 고르거나 왼쪽 상자·가운데 프리뷰에 놓으면 상단 컷 선택에만 들어가고 타임라인은 컷을 클릭해야 추가됩니다. 타임라인 썸네일 × 또는 Delete 키로 해당 프레임만 빼집니다. 상단 프리뷰는 화면 높이의 최대 44vh까지 정사각으로 자동 맞추고, 하단 시퀀서·내보내기 버튼은 줄어들지 않아 노트북에서도 잘리지 않습니다. 헤더 [💾 작업 저장]·[📂 작업 불러오기]와 자동 저장으로 이미지·프리셋·슬라이더·자막·폰트·꼬리 위치가 이 브라우저에 남고, 24시간 안이면 [이어하기] 한 번에 복원됩니다. 이어하기는 본체 그래픽·이모티콘 컷·내 PC 업로드 탭을 저장 당시 그대로 되돌리고, 저장하지 않고 닫거나 [새로 시작]하면 다음 진입은 본체 그래픽입니다. GIF/WebP가 끝나면 폴더 선택 창으로 저장합니다(미지원 브라우저는 기본 다운로드). 우측 [배경 합성 (Layer 0)]에서 투명(기본)·💡 화이트 스튜디오(오프화이트 단색+소프트 비네팅)·다크 스튜디오·그라데이션·내 이미지를 고르면 캐릭터 모션 뒤에만 깔리고, 투명으로 되돌리면 기존 알파 GIF가 그대로입니다. 바깥을 눌러도 창이 닫히지 않으며, ✕ 닫기 시 임시저장 확인 후 [이어하기]로 화이트 스튜디오를 포함한 배경까지 복원됩니다. 내 이미지가 4K여도 내보내기 전에 360×360으로 한 번만 줄여 캐시하므로 GIF가 3%에서 멈추지 않습니다. 분할 컷이 들어와도 중앙 열은 넘치지 않고, 하단 [클립 저장]·GIF·WebP 3버튼이 항상 보입니다. 좁은 개발 뷰에서는 1600 레이아웃을 축소해 브라우저 100%와 같은 3열로 보입니다. 툴바는 2줄(모션·파티클 / 자막 ON/OFF·입력·폰트·크기·색)이며, 자막 ON과 입력은 중앙 최상단 메인 프리뷰에도 같은 프레임에 합성됩니다. 끄거나 입력창이 비면 메인 캔버스와 GIF/WebP에 기본 예시 문구가 합성되지 않습니다. 우측 기본은 [모션 없음]이며, 선택한 10종 프리셋을 다시 누르면 선택이 해제됩니다. 파티클은 모션과 따로 움직입니다. 28컷을 쓸 때는 하단 시퀀서에서 컷을 타임라인에 쌓고 앞/뒤·삭제로 순서를 맞춥니다. 텍스트 모션(없음·바운스·쉐이크·펄스·타이핑)도 같은 프리뷰에 겹칩니다.',
          params: ['자막 ON/OFF', '폰트 10선', '꼬리 3점', '원본 복원', '타임라인 ×'],
        ...
        },
        {
          ...
          body: 'GIF/WebP 내보내기와 하단 [🚀 초고화질 무한루프 GIF 다운로드]는 같은 자동 투명 게이트·sanitizeExportFrames·360 Floyd-Steinberg 인코더를 씁니다. 진행률 팝업이 뜨고 프레임마다 메인 스레드를 양보합니다. 끝나면 OS 폴더 선택 창으로 저장되고(미지원 시 기본 다운로드) 임시 렌더 큐는 비워져 클립 라인에 복제본이 남지 않습니다. 마음에 들면 [클립 저장]으로 보관함 슬롯을 쌓고, 각 카드 [❌]로 개별 삭제하거나 [전체 비우기]로 초기화합니다. 작업 중이던 클립을 지우면 시퀀서는 기본 상태로 돌아갑니다.',
