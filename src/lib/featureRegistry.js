...
export const APP_FEATURES_REGISTRY = [
  ...
  feature({
    ...
    description: '2열 와이드 모달, 독립 플로팅 툴팁, 하단 35% 국소 텍스트 재채색, 슈퍼샘플링, JSZip 카카오 규격 점검',
    ...
    guideContent: {
      ...
      play: [
        ...
        },
        {
          ...
          body: '상단 [🧩 이모티콘 시트 분할기]는 가로가 긴 2열 스튜디오입니다. 왼쪽에서 시트 업로드·모드 A/B·ZIP·썸네일을 다루고, 오른쪽 대형 캔버스에서 1px 절단선과 외곽 재단선을 맞춥니다. 우측 상단 [−] [100%] [+] [⛶]로 확대하고 빈 곳을 드래그해 이동합니다. 버튼은 커지지 않고, 호버 시 다크 네온 설명 툴팁만 뜹니다. [텍스트 가독성]은 360×360 하단 35% 글자만 바꾸고 캐릭터 본체는 보존합니다.',
          params: ['2열 와이드 스튜디오', '줌/팬 · 화면맞춤', '독립 플로팅 툴팁', '하단 35% 텍스트만 재채색', '3× 슈퍼샘플링'],
          tip: '스크롤 없이 한 화면에 맞춰 작업하세요. 색을 바꿀 때는 캐릭터 얼굴이 아니라 하단 글자만 바뀌어야 합니다.',
